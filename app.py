...
app.layout = html.Div([
    ...
    html.Div([
        dcc.Upload(
            id='upload-data',
            children=html.Button("Upload .sav", style={"backgroundColor":"#00bfff","color":"#1e1e1e","border":"none","padding":"8px 12px","borderRadius":"8px","cursor":"pointer","fontWeight":"bold"}),
            style={'display':'inline-block','borderWidth':'0px','borderStyle':'none','borderRadius':'8px','textAlign':'left','margin':'10px'},
    ...
    html.Div([
        html.Div([
            ...
            dcc.Dropdown(
                id='district-dropdown', options=[], placeholder="All Districts", clearable=True,
        ...
        html.Div([
            ...
            dcc.Dropdown(
                ...
                placeholder="Select Variety", clearable=True,
                style={'backgroundColor':'#1E1E1E','color':'#00BFFF','border':'1px solid #00BFFF'}
            )
        ], style={"width":"45%","display":"inline-block","padding":"10px"})
...
@app.callback(
    ...
    [Input('upload-data','contents'), Input('district-dropdown','value'), Input('variety-dropdown','value')],
...
def update_dashboard(contents, selected_district, selected_variety, filename):
    ...
    if 'VARIETY' in filtered_df.columns and 'RATING' in filtered_df.columns and not filtered_df['RATING'].dropna().empty:
        ...
        bar_fig.update_layout(showlegend=False, plot_bgcolor='#262626', paper_bgcolor='#1e1e1e', font_color='#ffffff',
                              xaxis_tickangle=-45, bargap=0.5, bargroupgap=0.2)
    else:
        bar_fig = empty_fig
    ...
    if 'RATING' in filtered_df.columns and not filtered_df['RATING'].dropna().empty:
        ...
        dist_fig = px.histogram(
            plot_df, x='RATING', color='VARIETY_PLOT', barmode='overlay', nbins=5,
    ...
    unique_varieties_display = f"{present_count}/{len(ALLOWED_VARIETIES)}"
    cards=[
